...
export default function PodCard({ title, description, link }) {
  ...
  return (
    ...
    `}
      >
        <div className="absolute inset-0 bg-background text-foreground border-2 border-gray-600 dark:border-gray-600 rounded-lg flex items-center justify-center [backface-visibility:hidden]">
          <span className="text-2xl font-semibold font-rock-salt">{title}</span>
        </div>
        <div className="absolute inset-0 bg-background text-foreground border-2 border-gray-300 dark:border-gray-600 rounded-lg p-4 flex items-center justify-center [transform:rotateY(180deg)] [backface-visibility:hidden]">
          <p>{' "' + description + '" '}</p>
        </div>
      </div>
